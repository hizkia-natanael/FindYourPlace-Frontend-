        ...
        {reviews.length > 0 ? (
          reviews.map((review, index) => (
            <div key={index} className="bg-white shadow-sm rounded-md p-4 mb-4">
              ...
              <p className="text-sm text-[#a04925] font-semibold">
                {review.rating}
              </p>
            </div>
          ))
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPlaces.length > 0 ? (
              filteredPlaces.map((place, index) => (
                <div
                  key={index}
                  className="bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl hover:scale-105 transition-transform duration-300 cursor-pointer"
